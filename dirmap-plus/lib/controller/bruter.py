...
def scanModeHandler(db_file_name,db_id):
    '''
    ...
    # 其他模式处理，可同时加载
        ...
        if conf.crawl_mode:
            ...
            # 在iD模式下，如果对应的目标地址设置了cookie，那么就使用该cookie值
            if len(db_file_name) > 0:
                try:
                    conn = sqlite3.connect(db_file_name)
                    cursor = conn.cursor()
                    cookies = cursor.execute("select cookie from target_urls where id = ?",(db_id,))
                    cookies = list(map(lambda x:x[0],cookies))
                    if len(cookies) > 0 :
            ...
            try:
                ...
                outputscreen.error("[x] Crawler network connection error!plz check whether the target is err")
...
def responseHandler(response,db_file_name,db_id):
...
def worker(db_file_name, db_id):
    ...
    try:
        ...
        #3进入结果处理流程
        responseHandler(response,db_file_name, db_id)
...
def boss(db_file_name, db_id):
    ...
    while not tasks.all_task.empty():
        worker(db_file_name, db_id)

def bruter(db_file_name,url):
    # print(url)
    # print(url)
    # 从数据库中获取对应url的id
    db_id = 0
    ...
    if len(db_file_name) > 0:
        ...
        cursor = conn.cursor()
        try:
            ids = cursor.execute("select id from target_urls where url = ? limit 1",(url,))
    ...
    #加载payloads
    payloads.all_payloads = scanModeHandler(db_file_name,db_id)
    ...
    while not tasks.all_task.empty():
        all_task = [gevent.spawn(boss, db_file_name, db_id) for i in range(conf.request_limit)]
